...
(function () {
  ...
  BMapHelper.prototype.setMapInfo = function (data) {
    ...
    data.bmapMarker.addEventListener(
      'mouseover',
      (e) => {
        // 地图对象自定义属性，冻结
        ...
        // 获取弹框节点大小
        var originSize = data.bmapMarker.getIcon().size;
        var originDomWidth = originSize.width;
        var originDomHeight = originSize.height;
        // 地图偏移量
        var mapEl = e.target.getMap().getContainer();
  ...
  BMapHelper.prototype.setFullControl = function (data) {
    ...
    FullControl.prototype.initialize = function (map) {
      ...
      if (
        data.divHtml &&
        Array.isArray(data.divHtml) &&
        data.divHtml.length === 2
      ) {
        ...
        fullDivHtml =
          ...
          '<img src=' +
          (data.imgPath || that.imgPath + 'full.png') +
          ' title="全屏"/>' +
          '</div>';
        unfullDivHtml =
          '<div class="bmap-ctrl bmap-full">' +
          '<img src=' +
          (data.imgPath || that.imgPath + 'unfull.png') +
          ' title="恢复"/>' +
          '</div>';
      }
